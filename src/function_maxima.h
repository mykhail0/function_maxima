...
// Copy and swap idiom.
template<typename A, typename V>
auto FunctionMaxima<A, V>::operator=(
        FunctionMaxima other) noexcept -> FunctionMaxima & {
    other.swap(*this);
    return *this;
}

template<typename A, typename V>
auto FunctionMaxima<A, V>::make_point(const std::shared_ptr<A> &arg,
                                      const std::shared_ptr<V> &value) -> point_type {
